...
public class UpravljanjeTransakcije {
	...
	public void fatal(int vrstaNapake) {
		if(vrstaNapake == 0) {
        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_FATAL, "Transakcija ni mo�na!", "IBAN ra�un se ne ujema z nobenim drugim ra�unom v NMB Banki."));
		}
		if(vrstaNapake == 1) {
	        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_FATAL, "Transakcija ni mo�na!", "Prosimo poskusite ponovno kasneje."));
			}
		if(vrstaNapake == 2) {
	        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_FATAL, "Transakcija ni mo�na!", "Na ra�unu ni dovolj denarja."));
			}
		if(vrstaNapake == 3) {
	        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_FATAL, "Transakcija ni mo�na!", "Preglejte vpisane podatke in poskusite ponovno."));
			}
		if(vrstaNapake == 4) {
	        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_FATAL, "Polog ni mo�en!", "Transakcijski ra�un je zaprt/blokiran."));
			}
	}
	public String shraniTransakcijo(){
		//iskanje transakcijsih ra�unov, pla�nik po id-ju, prejemnik po TRR-ju
		...
		if(transakcijskiRacunPlacnika.getStanje().doubleValue() <= -100) {
			fatal(2);
			System.out.println("NA RA�UNU NI DOVOLJ DENARJA");
			return "nakazi";
		}
